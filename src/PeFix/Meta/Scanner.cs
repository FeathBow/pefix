...
public static class Scanner
{
    public static ScanReport Scan(string path)
    {
        ...
        var deps = DepIndex.Build(results);
        VerConflict[] conflicts = deps.FindConflicts(results);
        MissingRef[] missingRefs = deps.FindMissing(results);
        DupProvider[] dupProviders = DepIndex.FindDuplicates(results);
